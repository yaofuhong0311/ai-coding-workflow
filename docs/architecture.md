...
（OpenClaw 为私有工具，可替换为任意 cron + 通知方案）
...
Swarm 模式由 Queen（主 Claude 进程）负责读取 plan.md、拆分任务、通过 CC 内置 Agent 工具并行分发给多个子 Agent 执行。Ruflo hive-mind 负责 Worker 注册和 claims 防冲突，CC 内置 Agent 子进程负责真正的并行执行。详细流程见 README.md 的"Swarm 并行"章节。
...
以下是 `~/.claude/settings.json` 中与本工作流相关的 hooks 配置示意结构（非真实命令）：
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "command": "（示意）编码任务 → 注入 ruflo memory_search 指令；检测 plan.md 任务数 >= 2 → 注入 Swarm 强制指令",
            "type": "command"
          }
        ]
      }
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "command": "（示意）ruflo hooks metrics → 确认 MCP 挂载状态",
            "type": "command"
          }
        ]
      }
...
> 完整的实际 bash 命令见 ~/.claude/settings.json，建议直接参考该文件。python-quality-gate.sh 的实际脚本见 ~/.claude/hooks/python-quality-gate.sh。
